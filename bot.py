...
# — Московская часовая зона для APScheduler —
...
SCHEDULE = [
    ...
    { "time": "21:55", "text": '❌ <a href="https://mostbet2.com/admin/payout-route/list?filter%5Bcurrency%5D%5Btype%5D=&filter%5BuserRole%5D%5Btype%5D=&filter%5BuserRole%5D%5Bvalue%5D=&filter%5BpayoutMethod%5D%5Btype%5D=&filter%5BpayoutMethod%5D%5Bvalue%5D=khalti_birpay&filter%5Bactive%5D%5Btype%5D=&filter%5Bactive%5D%5Bvalue%5D=&filter%5BalwaysSentToAntiFraud%5D%5Btype%5D=&filter%5BalwaysSentToAntiFraud%5D%5Bvalue%5D=&filter%5B_page%5D=1&filter%5B_sort_by%5D=id&filter%5B_sort_order%5D=DESC&filter%5B_per_page%5D=32">Выключить метод выплат Khalti_birpay в админке</a>' },
...
# — Error-handler игнорирует только Conflict —
...
def all_notifications(update: Update, context: CallbackContext):
    now = datetime.datetime.now(MSK).date()
    today_items = []
    for item in SCHEDULE:
        hh, mm = map(int, item["time"].split(":"))
...
# — Планируем все боевые уведомления (без тестовых ежечасных) —
...
def main():
    ...
    # 4) Планирование боевых уведомлений
